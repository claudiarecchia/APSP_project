    ...
    for ex in executions:
        if graph_type[i] in ex[0]:
            if "F" in ex[1]:
                ...
                dijkstra_parallel.append([ex[0], float(ex[2])])
            elif "D" in ex[1]:
...
# plt.show()
#
# plt.plot(dijkstra[24][2], dijkstra[24][1], 'r', marker='o', label=dijkstra[24][0])
# plt.plot(dijkstra[4][2], dijkstra[4][1], 'b', marker='o', label=dijkstra[4][0])
# plt.plot(dijkstra[9][2], dijkstra[9][1], 'y', marker='o', label=dijkstra[9][0])
# plt.plot(dijkstra[14][2], dijkstra[14][1], 'm', marker='o', label=dijkstra[14][0])
# plt.plot(dijkstra[19][2], dijkstra[19][1], 'g', marker='o', label=dijkstra[19][0])
# plt.xlabel('Numero nodi')
# plt.ylabel('Tempo di esecuzione')
# plt.title("Relazione Dijkstra: tempo di esecuzione, numero archi")
# plt.legend()
...
# plt.plot([dijkstra[10][2], dijkstra[11][2], dijkstra[12][2], dijkstra[13][2], dijkstra[14][2]], [dijkstra[10][1], dijkstra[11][1], dijkstra[12][1], dijkstra[13][1], dijkstra[14][1]], 'pink', marker='o', label=(dijkstra[10][0])[:8])
# plt.plot([dijkstra[15][3], dijkstra[16][3], dijkstra[17][3], dijkstra[18][3], dijkstra[19][3]], [dijkstra[15][1], dijkstra[16][1], dijkstra[17][1], dijkstra[18][1], dijkstra[19][1]], 'lightblue', marker='o', label=(dijkstra[15][0])[:7])
# plt.xlabel('Numero archi')
# plt.ylabel('Tempo di esecuzione')
# plt.title("Relazione Dijkstra: tempo di esecuzione, numero archi")
# plt.legend()
...
# plt.plot( [dijkstra[10][1], dijkstra[11][1], dijkstra[12][1], dijkstra[13][1], dijkstra[14][1]], [dijkstra[10][2], dijkstra[11][2], dijkstra[12][2], dijkstra[13][2], dijkstra[14][2]], 'blue', marker='o', label=(dijkstra[10][0])[:8])
# plt.plot( [dijkstra[15][1], dijkstra[16][1], dijkstra[17][1], dijkstra[18][1], dijkstra[19][1]], [dijkstra[15][3], dijkstra[16][3], dijkstra[17][3], dijkstra[18][3], dijkstra[19][3]], 'lightblue', marker='o', label=(dijkstra[15][0])[:7])
...
for element in dijkstra:
    if element[0] == "g_ER_p1_2000":
        dijkstra.remove(element)

# speedup
for i in range(len(dijkstra_parallel)):
    report_speedup(dijkstra[i][0], dijkstra[i][1] / dijkstra_parallel[i][1])